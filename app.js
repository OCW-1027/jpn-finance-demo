// ===== STORAGE =====
const DKEY='jpndemo_data', SKEY='jpndemo_settings';
...
function fetchRate(){
  ...
  fetch('https://api.exchangerate-api.com/v4/latest/USD')
    ...
    .then(data=>{
      ...
      localStorage.setItem('jpndemo_settings',JSON.stringify(SET));
...
<body>
<div style="text-align:center;margin-bottom:16pt"><div style="font-size:20pt;font-weight:bold;color:#1e3a5f">JPNデモ 자금운용보고서</div><div style="font-size:11pt;color:#666;margin-top:4pt">${rptDt()} 기준</div></div>
...
<tr><td style="${S}background:#f5f5f5;font-weight:bold">법인계좌잔액---(1)</td><td style="${HB}background:#f5f5f5">${fm(c.bb)}</td><td style="${S}background:#f5f5f5;color:#888">銀行</td></tr>
...
<tr><td style="${S}font-weight:bold">증권계좌잔액---(2)</td><td style="${HB}">${fm(c.secBal)}</td><td style="${S}color:#888">証券会社</td></tr>
...
<br><p style="color:#999;font-size:9pt;text-align:center">본 보고서는 JPNデモ 재무관리 프로그램에서 자동 생성되었습니다.</p>
</body></html>`;

  const blob=new Blob([wordHTML],{type:'application/msword'});
  const url=URL.createObjectURL(blob);const a=document.createElement('a');
  a.href=url;a.download='JPNデモ_운용보고서_'+new Date().toISOString().slice(0,10)+'.doc';
...
function rFS(){
  ...
  '<div id="TC"><div class="pn" style="padding:18px;max-width:680px"><div style="text-align:center;margin-bottom:16px"><div style="font-size:16px;font-weight:700">손 익 계 산 서 (잠정)</div><div style="font-size:12px;color:#64748b">JPNデモ株式会社 (단위:엔)</div></div>'+
...
function rRpt(){const c=calc();
  ...
  return '<div style="max-width:1100px" id="rptContent">'+
    '<div contenteditable="true" style="text-align:center;margin-bottom:20px"><div style="font-size:22px;font-weight:700;color:#1e3a5f">JPNデモ 자금운용보고서</div><div style="font-size:13px;color:#64748b;margin-top:4px">'+rptDt()+' 기준</div></div>'+
    ...
    '<tr class="a" style="font-weight:700"><td>법인계좌잔액---(1)</td><td class="r m b">'+fm(c.bb)+'</td><td class="mu">銀行</td></tr>'+
    '<tr><td>증권예수금</td><td class="r m">'+fm(c.secDep)+'</td><td></td></tr>'+
    '<tr class="a"><td>유가증권평가액</td><td class="r m">'+fm(c.allMv)+'</td><td></td></tr>'+
    '<tr style="font-weight:700"><td>증권계좌잔액---(2)</td><td class="r m b">'+fm(c.secBal)+'</td><td class="mu">証券会社</td></tr>'+
...
function exportBackup(){
  ...
  a2.href=url;
  a2.download='jpndemo_backup_'+new Date().toISOString().slice(0,10)+'.json';
...
function exportGLExcel(){
  ...
  html+='<h2 style="font-size:14pt;color:#1e3a5f">JPNデモ株式会社　総勘定元帳</h2>';
  ...
  a2.href=url;a2.download='JPNデモ_総勘定元帳_'+new Date().toISOString().slice(0,10)+'.xls';
...
<h1>損 益 計 算 書</h1>
<div class="sub">JPNデモ株式会社<br>自 令和7年6月2日（設立日）至 令和8年4月2日（暫定）<br>（単位：円）</div>
...
<div class="stamp"><div class="s1">決裁</div><div class="s2">本人専決</div></div>
<h1>貸 借 対 照 表</h1>
<div class="sub">JPNデモ株式会社<br>令和8年4月2日現在（暫定）<br>（単位：円）</div>
...
<div class="footer">JPNデモ株式会社 財務管理システム ｜ 出力日: ${dateStr} ｜ 本書は暫定値に基づく参考資料です</div>
</body></html>`;

  const blob=new Blob([html],{type:'application/msword'});
  const url=URL.createObjectURL(blob);const a2=document.createElement('a');
  a2.href=url;a2.download='JPNデモ_財務諸表_'+new Date().toISOString().slice(0,10)+'.doc';
  document.body.appendChild(a2);a2.click();document.body.removeChild(a2);URL.revokeObjectURL(url);
}
...
function rTxTab(){
  ...
  return '<div class="pn" style="padding:18px;max-width:520px">'+
    ...
    '<div style="text-align:center;font-size:10px;color:#64748b;margin-bottom:14px">JPNデモ株式会社 (자본금1천만엔, 도쿄도, 소규모법인)</div>'+
