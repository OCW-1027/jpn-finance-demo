...
fbReady = false; // JPN Demo: Firebase disabled
...
<div class="footer">JPNデモ 財務管理システム ｜ 出力日: ${dateStr} ｜ 本書は暫定値に基づく参考資料です</div>
</body></html>`;

  const blob=new Blob([html],{type:'application/msword'});
  const url=URL.createObjectURL(blob);const a2=document.createElement('a');
  a2.href=url;a2.download='JPNDemo_財務諸表_'+new Date().toISOString().slice(0,10)+'.doc';
  document.body.appendChild(a2);a2.click();document.body.removeChild(a2);URL.revokeObjectURL(url);
}
...
function rTxTab(){
  ...
  return '<div class="pn" style="padding:18px;max-width:520px">'+
    ...
    '<div style="text-align:center;font-size:10px;color:#64748b;margin-bottom:14px">JPNデモ株式会社 (자본금5백만엔, 도쿄도, 소규모법인)</div>'+
